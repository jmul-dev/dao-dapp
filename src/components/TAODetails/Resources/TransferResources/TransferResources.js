		...
		switch (formData.resourceType) {
			case "ETH":
				...
				if (balance.lt(formData.amount)) {
					this.setState({ error: true, errorMessage: "TAO has insufficient ETH balance", formLoading: false });
					return false;
				}
				nameTAOVault.transferEth(id, formData.recipient, formData.amount, { from: accounts[0] }, (err, transactionHash) => {
					if (err) {
						...
						waitForTransactionReceipt(transactionHash)
							.then(async () => {
								await this.props.getTAOResources();
			...
			case "AO":
				...
				if (balance.lt(formData.amount)) {
					this.setState({ error: true, errorMessage: "TAO has insufficient AO balance", formLoading: false });
					return false;
				}
			...
			case "ERC20":
				...
				try {
					...
					if (balance.lt(formData.amount)) {
						this.setState({
							error: true,
							errorMessage: `TAO has insufficient ${tokenERC20Name} (${tokenERC20Symbol}) balance`,
							formLoading: false
						});
						return false;
					}
